...
server <- function(input, output){
  ...
  dt <- data.frame(code = c(1, 2, 3),
                   data.set = c("A549", "MCF7", "RKO"),
                      file.location = c("data/A549_deep_proteome.csv", "data/MCF7_deep_proteome.csv", "data/RKO_deep_proteome.csv"))
    ...
    if(is.character(ds)){
      ...
      if(input$radio == 4 & length(input$checkGroup) > 0){
        for(i in 1:length(input$checkGroup)){
          ...
          colnames(mer) <- paste(as.character(dt$data.set[which(dt$code == input$checkGroup[i])]), colnames(mer), sep = ".")
